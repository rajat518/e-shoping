...
pw.println("count"+mcount);
	...
	//pw.println("select num_of_items,buffer from  products where pro_id='"+pid+"'  and branch_id='"+bid+"'");
       pw.print(bid);
	rs15=st4.executeQuery("select num_of_items,buffer from products where pro_id='"+pid+"'");
	
        rs15.next();
	qty2=rs15.getInt(1);
        pw.print("ddddddddddd");
        pw.print(qty2);
	buff=rs15.getInt("buffer");
    qty2=qty2-buff; 
	...
	if(qty2 >= qty1)
		{
				 pw.println("if1"+qty2);
				  ...
				  // pw.println("update products set num_of_items="+x+" where branch_id ='"+bid+"' and pro_id='"+pid+"'");
				   st6.executeUpdate("update products set num_of_items="+x+" where pro_id='"+pid+"'");
...
else if((qty2 + buff)>=qty1)
		{
		            ...
		            st4.executeUpdate("update products set num_of_items="+y+" where pro_id='"+pid+"'");
					//pw.println("update orderdetails set DPRO_QTY='"+qty1+"' where orderid  ='"+orderid+"'");
				   //st4.executeUpdate("update orderdetails set DPRO_QTY='"+qty1+"' where orderid  ='"+orderid+"'");
				   con.commit();
					c++;
...
rs2=st6.executeQuery("select  pro_name,supp_id,max from products where pro_id='"+pid+"'");
rs2.next();
 pname=rs2.getString("pro_name");
 sid=rs2.getString("supp_id");
 kmax = Integer.parseInt(rs2.getString("max"));
 qty=kmax;
 pw.println(" SELECT count(pro_id) from sup_order where  pro_id='"+pid+"'"); 
		String query=("SELECT count(pro_id) from sup_order where  pro_id='"+pid+"'");
		ResultSet rs=ps.executeQuery(query);
		rs.next();
		 count=rs.getInt("count(pro_id)");
        if(count!=0)
		{
         pw.println("select quantity from sup_order where  pro_id='"+pid+"') ");
		rs=ps2.executeQuery("select quantity from sup_order where  pro_id='"+pid+"'");
		while(rs.next())
		{		
		qty += rs.getInt("quantity");
		pw.println("update sup_order set quantity="+qty+" pro_id='"+pid+"' and branch_id='"+bid+"'");
		rw=ps.executeUpdate("update sup_order set quantity="+qty+" pro_id='"+pid+"'");
        con.commit();		
		}
		}      
		...
		con.commit();
...
else
				{
							 ...
							 con.commit();
							 pw.println("select Max from products where  pro_id='"+pid+"'");
							 rs14=st4.executeQuery("select Max from products where  pro_id='"+pid+"'"); 
							rs14.next();
                             ...
                             if(qty1>max)
					{
							  ...
							  req.setAttribute("f",1);
							   ...
							   rd= req.getRequestDispatcher("./Supplierorder");
  							   rd.forward(req,res);*/
 pw.println("select  pro_name,supp_id,max from products where pro_id='"+pid+"'");
rs2=st6.executeQuery("select  pro_name,supp_id,max from products where pro_id='"+pid+"'");
rs2.next();
 pname=rs2.getString("pro_name");
 sid=rs2.getString("supp_id");
...
if(qty2 > 0)
						{
	 kmax = (qty1-qty2); 
	 pw.println("update products set num_of_items="+buff+" where pro_id='"+pid+"'");
     st4.executeUpdate("update products set num_of_items="+buff+" where pro_id='"+pid+"'");
