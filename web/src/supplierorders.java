...
import java.text.*;
import java.sql.*;
public class supplierorders extends HttpServlet
{
        Connection con=null;
	Statement ps=null;	
	ResultSet rs=null;
		 ...
		 pw.println("<html>");
			...
			pw.println("<td><img src='s0.GIF' width=1000 height=135></td></tr>");
            	//pw.println("<form  name=f2  action=./empdel method=get>");
				 ...
				 pw.println("ok");
           pw.println("<p><table cellpadding=1 cellspacing=1 border=1><th>pro_name</th><th>pro_id</th><th>branch_id</th><th>supp_id</th><th>quantity</th><th>dd</th></tr>");
		int i=1;
...
rs=ps.executeQuery("select pro_name,pro_id,branch_id,supp_id,quantity,dd from sup_order");
		...
		while(rs.next())
          ...
          pw.println("<tr bgcolor=white><td><td>"+pro_name+"</td><td>"+pro_id+"</td><td>"+branch_id+"</td><td>"+supp_id+"</td><td>"+quantity+"</td><td>"+dd+"</td></tr>");
