...
{ alert("Please Enter Valid Address");f.state.value="";f.state.focus();return false;}
else if(f.country.value.length==0)
{ alert("Please Enter Valid Address");f.country.value="";f.country.focus();return false;}
else if(f.country.value=="country")
{ alert("Please Enter Valid Address");f.country.value="";f.country.focus();return false;}
...
int p=0,q=0,tqty=0;
String name1="",address="",city="",state="",country="",name2="";
int pincode=0;
double sum=0.00;
rs=stmt.executeQuery("select coname,address,city,state,pin from customer where cust_id='"+session.getAttribute("cust_id")+"'");
while(rs.next())
{
...
rs2=st.executeQuery("select distinct cart.pr_qty,product.price from product,cart where cart.cust_id='"+session.getAttribute("cust_id")+"' and cart.pid=product.pid");
...
<%}else { %>
<br><br><center><b>INVALID CART SELECTED....<a href="home.jsp">PLEASE ADD ITEMS TO CART</a></b></center><br><br>
<%}%>
</body>
</html>
